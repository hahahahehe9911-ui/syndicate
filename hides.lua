...
local function createScriptNameLabel(nameLabel, billboard)
    ...
    local existingFrame = billboard:FindFirstChild("JayZFrame")
    ...
    local JayZFrame = Instance.new("Frame")
    JayZFrame.Name = "JayZFrame"
    JayZFrame.Size = nameFrame.Size
    JayZFrame.Position = originalNamePos
    JayZFrame.BackgroundTransparency = 1
    JayZFrame.Parent = billboard
    
    local scriptLabel = nameLabel:Clone()
    scriptLabel.Name = "JayZLabel"
...
local function removeAllScriptNames()
    ...
    local JayZFrame = overhead:FindFirstChild("JayZFrame")
    if JayZFrame then
        ...
        if obj:IsA("TextLabel") then
            ...
            if originalText and originalText ~= "" then
                if obj.Name == "Header" then
                    if not overhead:FindFirstChild("JayZFrame") then
...
print("[Hide Stats Module] Loaded successfully!")
